...
if __name__ == '__main__':
    ...
    #"/home/explainable/src/dataset/segmin/segmim/segmim_08189.jpg"
    #args = get_args()
    model=vgg16_bn()
    model.load_state_dict(torch.load("/home/explainable/src/Grad-cam-compress/compressed_model/vgg16_xgrad_high_radio/epoch: 446 91.3725.pth"))
    print(model)
    grad_cam = GradCam(model=model, 
                        target_layer_names=["40"], use_cuda=True)

    img = cv2.imread("/home/explainable/src/Grad-cam-compress/test_1.jpg", 1)
    img = np.float32(cv2.resize(img, (224, 224))) / 255
    #img = np.float32(cv2.resize(img, (224, 224))) / 255
    #print(img1)
    #mask=(img1[:,:,0]!=0.99607843)
    #print(mask)
    #cv2.imwrite('mask' + ".jpg", np.uint8(255 * mask*img))
    #print(gc)
    ...
    show_cam_on_image(img, cam, 'cam_1')
    #show_cam_on_image(img, X_cam, 'X_cam_1')
